...
export const DoctorPortal: React.FC = () => {
  ...
  const loadDoctorData = async (docId: string) => {
    try {
      ...
      if (res.ok && res.data) {
        ...
        setKpis({
          count: (list.length || 4) + bookings.length,
          intakes: (res.data.pending_questionnaires_count || 3) + bookings.length,
        });
      }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              City Memorial Hospital &bull; Consultation Duration: 30 Mins &bull; NPI #198234812
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
          ...
          <div className="text-2xl font-black text-white mt-1">
            {appointments.length + bookings.length} Patients
          </div>
          <div className="text-[11px] text-emerald-400 font-semibold mt-0.5">
            &bull; 100% EHR Verified
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
          ...
          <div className="text-[11px] text-sky-400/80 font-medium mt-0.5">
            &bull; Structured answers saved
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
          ...
          <div className="text-2xl font-black text-white mt-1">
            {Math.min(100, Math.round(((appointments.length + bookings.length) / 12) * 100))}%
          </div>
          <div className="text-[11px] text-slate-500 mt-0.5">Continuous auto-balancing</div>
        </div>
      ...
      {/* Active Clinical Encounter Modal */}
      {encounterAppt && (
        <EncounterModal
          booking={{
            ...
            scheduled_time: encounterAppt.time || '10:00 AM',
